...
- **Pull request number** — e.g., #42. The agent fetches the diff and reviews only changed lines in context.
...
- **Classify severity explicitly.** Use Blocking / Non-Blocking / Nit levels so the author knows what must be fixed before merging versus what is a suggestion.
