...
exec > >(tee "$REPO_ROOT/workbench.log") 2>&1
...
if [[ "$os_type" == "macos" ]]; then
  inventory="$SCRIPT_DIR/inventory-macos.yml"
  playbook="$SCRIPT_DIR/site-macos-arm64.yml"
elif [[ "$os_type" == "windows" ]]; then
  inventory="$SCRIPT_DIR/inventory-windows.yml"
  playbook="$SCRIPT_DIR/site-windows-amd64.yml"
fi
...
ansible-playbook \
  -i "$inventory" \
  "$playbook" \
  -e "devtools_repo_root=$REPO_ROOT"
